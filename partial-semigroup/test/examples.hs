...
prop_tuple_Just =
  example $
    let x = (Left "ab", Right "hi")
        y = (Left "cd", Right "jk")
     in x <>? y === Just (Left "abcd", Right "hijk")
...
prop_partialZip1_Just =
  example $
    let xs = Left "a" :| [Left "b", Right "c"]
        ys = Left "1" :| [Left "2", Right "3"]
     in partialZip1 xs ys === Just (Left "a1" :| [Left "b2", Right "c3"])

prop_partialZip1_item'mismatch :: Property
prop_partialZip1_item'mismatch =
  example $
    let xs = Left "a" :| [Left "b", Right "c"]
        ys = Left "1" :| [Right "2", Right "3"]
     in partialZip1 xs ys === Nothing

prop_partialZip1_length'mismatch :: Property
prop_partialZip1_length'mismatch =
  example $
    let xs = Left "a" :| [Left "b", Right "c"]
        ys = Left "1" :| [Left "2"]
     in partialZip1 xs ys === Nothing
...
prop_Total_partialConcat =
  example $
    let f = getProduct . unTotal
        g = Total . Product
     in (fmap f . partialConcat . fmap g) [1 .. 4] === Just 24

prop_AppendLeft_Just :: Property
prop_AppendLeft_Just =
  example $
    let x = AppendLeft (Left "ab")
        y = AppendLeft (Left "cd")
     in x <>? y === Just (AppendLeft (Left "abcd"))

prop_AppendLeft_Nothing :: Property
prop_AppendLeft_Nothing =
  example $
    let x = AppendLeft (Right "ab")
        y = AppendLeft (Right "cd")
     in x <>? y === Nothing

prop_AppendLeft_groupAndConcat :: Property
prop_AppendLeft_groupAndConcat =
  example $
    let xs = [Left "a", Left "b", Right "c", Right "d", Left "e", Left "f"]
        f = fmap unAppendLeft . groupAndConcat . fmap AppendLeft
     in f xs === [Left "ab", Right "c", Right "d", Left "ef"]

prop_AppendRight_Just :: Property
prop_AppendRight_Just =
  example $
    let x = AppendRight (Right "ab")
        y = AppendRight (Right "cd")
     in x <>? y === Just (AppendRight (Right "abcd"))

prop_AppendRight_Nothing :: Property
prop_AppendRight_Nothing =
  example $
    let x = AppendRight (Left "ab")
        y = AppendRight (Left "cd")
     in x <>? y === Nothing

prop_AppendRight_groupAndConcat :: Property
prop_AppendRight_groupAndConcat =
  example $
    let xs = [Left "a", Left "b", Right "c", Right "d", Left "e", Left "f"]
        f = fmap unAppendRight . groupAndConcat . fmap AppendRight
     in f xs === [Left "a", Left "b", Right "cd", Left "e", Left "f"]
